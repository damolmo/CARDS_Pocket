...
class CardsData{
  ...
  static insertCards(){
    ...
    for (String category in cards.keys){
      for (String card in cards[category].keys){
        Cards currentCard = Cards(
          name: card,
          color: cards[category][card][0],
          value: cards[category][card][1],
          uri: cards[category][card][2]);
  ...
  static List<Cards> webAssets(){
    ...
    for (String color in cards_web.keys){
      for (String card in cards_web[color].keys){
        Cards currentCard =  Cards(name: card, color: cards_web[color][card][0], value: cards_web[color][card][1], uri: cards_web[color][card][2]);
        cardsFinal.add(currentCard);
        print(currentCard.uri);
      }
    }
